...
<body class="flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto relative pb-20 md:pb-0">
        ...
        <header class="h-16 bg-terminal-panel border-b border-gray-800 flex items-center justify-between px-4 md:px-6 shrink-0 sticky top-0 z-20">
            <div class="flex items-center space-x-4 md:space-x-6">
                ...
                <form method="GET" action="" class="flex items-center bg-black border border-gray-700 rounded px-2 py-1">
                    <select name="switch_portfolio" onchange="this.form.submit()" class="bg-black text-electric-blue font-mono text-xs md:text-sm outline-none font-bold cursor-pointer">
                        <option value="Personal" <?= $active_portfolio === 'Personal' ? 'selected' : '' ?>>PERSONAL</option>
                        <option value="Master_Joint" <?= $active_portfolio === 'Master_Joint' ? 'selected' : '' ?>>MANAGED FUNDS</option>
                    </select>
                </form>
            </div>
        ...
        <div class="p-4 md:p-6 flex-1 flex flex-col">
            <div class="flex justify-between items-end border-b border-gray-800 pb-2 mb-6 shrink-0 mt-2">
                <h1 class="text-lg md:text-xl font-bold font-mono text-gray-400">PORTFOLIO_OVERVIEW</h1>
            </div>
            
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 shrink-0">
                <div class="bg-terminal-panel p-4 md:p-5 rounded border border-gray-800 shadow-lg">
                    <div class="text-gray-500 text-[10px] md:text-xs font-mono mb-2">INITIAL (USC)</div>
                    <div class="text-lg md:text-2xl number-format"><?= number_format($metrics['total_initial_cent'], 2, '.', ',') ?></div>
                </div>
                <div class="bg-terminal-panel p-4 md:p-5 rounded border border-gray-800 shadow-lg">
                    <div class="text-gray-500 text-[10px] md:text-xs font-mono mb-2">CURRENT (USC)</div>
                    <div class="text-lg md:text-2xl number-format <?= $metrics['current_balance_cent'] >= $metrics['total_initial_cent'] ? 'text-neon-green' : 'text-neon-red' ?>">
                        <?= number_format($metrics['current_balance_cent'], 2, '.', ',') ?>
                    </div>
                </div>
                <div class="bg-terminal-panel p-4 md:p-5 rounded border border-gray-800 shadow-lg">
                    <div class="text-gray-500 text-[10px] md:text-xs font-mono mb-2">NET PNL (USC)</div>
                    <div class="text-lg md:text-2xl number-format <?= $metrics['total_pnl_cent'] >= 0 ? 'text-neon-green' : 'text-neon-red' ?>">
                        <?= $metrics['total_pnl_cent'] >= 0 ? '+' : '' ?><?= number_format($metrics['total_pnl_cent'], 2, '.', ',') ?>
                    </div>
                </div>
                <div class="bg-terminal-panel p-4 md:p-5 rounded border border-gray-800 shadow-lg">
                    <div class="text-gray-500 text-[10px] md:text-xs font-mono mb-2">GROWTH / ROI</div>
                    <div class="text-lg md:text-2xl number-format <?= $metrics['growth_percentage'] >= 0 ? 'text-neon-green' : 'text-neon-red' ?>">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 shrink-0">
                <div class="bg-terminal-panel p-4 rounded border border-gray-800 shadow-lg relative h-72 md:h-80">
                    <h2 class="text-gray-500 text-xs font-mono mb-2 absolute top-4 left-4 z-10">EQUITY CURVE</h2>
                    <canvas id="equityChart"></canvas>
                </div>
                
                <div class="bg-terminal-panel p-4 rounded border border-gray-800 shadow-lg relative h-72 md:h-80">
                    <h2 class="text-gray-500 text-xs font-mono mb-2 absolute top-4 left-4 z-10">DAILY PNL VS DD</h2>
    ...
    <div class="md:hidden fixed bottom-0 w-full bg-black/80 backdrop-blur-lg border-t border-gray-800 z-50 flex justify-around items-center pt-2 pb-safe" style="padding-bottom: env(safe-area-inset-bottom, 12px);">
        <a href="index" class="flex flex-col items-center p-2 text-neon-green">
            <svg class="w-6 h-6 mb-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" /></svg>
            <span class="text-[10px] font-mono font-bold">Dash</span>
        </a>
        <a href="input" class="flex flex-col items-center p-2 text-gray-500 hover:text-white transition-colors">
    ...
    <script>
        ...
        function openMoreMenu() {
            moreSheet.classList.remove('hidden');
            moreSheet.classList.add('flex');
            // Delay tipis agar transisi CSS berjalan mulus
            setTimeout(() => {
                moreContent.classList.remove('translate-y-full');
            }, 10);
        }
        ...
        function closeMoreMenu() {
            moreContent.classList.add('translate-y-full');
            // Tunggu animasi turun selesai, baru sembunyikan overlay
            setTimeout(() => {
                moreSheet.classList.add('hidden');
                moreSheet.classList.remove('flex');
            }, 300);
        ...
        // Tutup menu jika area gelap / overlay ditekan
        if(moreSheet) {
            moreSheet.addEventListener('click', (e) => {
                if (e.target === moreSheet) closeMoreMenu();
            });
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($journal->addDailyLog($date, $account_id, $pnl_cent, $max_dd_cent, $remarks)) {
        $message = "<div class='bg-neon-green text-terminal-black font-mono px-4 py-2 rounded mb-6 font-bold'>[SUCCESS] LOG ENTRI BERHASIL DISIMPAN KE DATABASE.</div>";
    } else {
        $message = "<div class='bg-neon-red text-white font-mono px-4 py-2 rounded mb-6 font-bold'>[ERROR] GAGAL MENYIMPAN DATA.</div>";
    }
}
?>
<!DOCTYPE html>
...
<body class="flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto relative pb-20 md:pb-0">
        ...
        <div class="p-4 md:p-6 flex-1 flex flex-col items-center">
            <div class="w-full max-w-2xl">
                <h1 class="text-lg md:text-xl font-bold mb-6 font-mono text-gray-400 border-b border-gray-800 pb-2 mt-2">DATA_ENTRY_MODULE</h1>
                
                <?= $message ?>

                <div class="bg-terminal-panel p-5 md:p-6 rounded border border-gray-800 shadow-lg mb-6 shrink-0">
                    <form method="POST" action="">
                        ...
                        <div class="grid grid-cols-2 gap-4 md:gap-6 mb-4">
                            <div>
                                <label class="block text-gray-500 text-xs font-mono mb-2">PNL HARIAN (USC)</label>
                            ...
                            <div>
                                <label class="block text-gray-500 text-xs font-mono mb-2">MAX DRAWDOWN (USC)</label>
    ...
    <div class="md:hidden fixed bottom-0 w-full bg-black/80 backdrop-blur-lg border-t border-gray-800 z-50 flex justify-around items-center pt-2 pb-safe" style="padding-bottom: env(safe-area-inset-bottom, 12px);">
        ...
        <a href="input" class="flex flex-col items-center p-2 text-neon-green">
            <svg class="w-6 h-6 mb-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
            <span class="text-[10px] font-mono font-bold">Entry</span>
        </a>
        <a href="clients" class="flex flex-col items-center p-2 text-gray-500 hover:text-white transition-colors">
...
<!-- end file /partner_dashboard.php -->
...
<body class="flex h-screen overflow-hidden">
    ...
    <aside id="sidebar" class="hidden md:flex bg-terminal-panel w-64 border-r border-gray-800 sidebar-transition flex-col z-10 relative shrink-0">
        ...
        <nav class="flex-1 px-2 py-4 space-y-2 mt-2 flex flex-col justify-between overflow-y-auto">
            <div>
                ...
                <a href="report" class="group flex items-center py-2 px-3 bg-gray-800 rounded border-l-2 border-neon-green text-neon-green whitespace-nowrap overflow-hidden mb-2">
                    <svg class="w-5 h-5 shrink-0 transition-colors" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg>
                    <span class="nav-text ml-3">Annual Report</span>
                </a>
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto relative pb-20 md:pb-0">
        ...
        <div class="p-4 md:p-6 flex-1 flex flex-col">
            <div class="flex justify-between items-end border-b border-gray-800 pb-2 mb-6 shrink-0 mt-2">
                <h1 class="text-lg md:text-xl font-bold font-mono text-gray-400">ANNUAL_PNL_MATRIX_<?= $selected_year ?></h1>
                
                <form method="GET" action="" class="flex items-center space-x-2">
                    <span class="text-gray-500 font-mono text-[10px] md:text-xs">YEAR:</span>
                    <select name="year" onchange="this.form.submit()" class="bg-black border border-gray-700 text-electric-blue font-mono text-xs md:text-sm px-2 py-1 rounded outline-none focus:border-electric-blue">
                        <?php 
                        $start_year = 2024;
            ...
            <div class="bg-terminal-panel rounded border border-gray-800 shadow-lg overflow-x-auto shrink-0 mb-6">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-900 border-b border-gray-700 font-mono text-[10px] md:text-xs text-gray-400">
                            <th class="p-3 md:p-4 uppercase tracking-wider">Month</th>
                            <th class="p-3 md:p-4 uppercase tracking-wider text-right">Net PNL (USC)</th>
                            <th class="p-3 md:p-4 uppercase tracking-wider text-right">Max DD (USC)</th>
                        </tr>
                    </thead>
                    <tbody class="font-mono text-xs md:text-sm">
                        ...
                        ?>
                        <tr class="border-b border-gray-800 hover:bg-gray-800 transition-colors">
                            <td class="p-3 md:p-4 text-gray-300"><?= $month_name ?></td>
                            <td class="p-3 md:p-4 text-right <?= $pnl_color ?>">
                                <?= $pnl_cent > 0 ? '+' : '' ?><?= number_format($pnl_cent, 2, '.', ',') ?>
                            </td>
                            <td class="p-3 md:p-4 text-right <?= $dd_cent < 0 ? 'text-neon-red' : 'text-gray-500' ?>">
                    ...
                    <tfoot>
                        <tr class="bg-gray-900 border-t border-gray-700 font-mono font-bold text-xs md:text-sm">
                            <td class="p-3 md:p-4 text-electric-blue">TOTAL YTD</td>
                            <td class="p-3 md:p-4 text-right <?= $total_annual_pnl > 0 ? 'text-neon-green' : 'text-neon-red' ?>">
    ...
    <div id="mobile-more-sheet" class="md:hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden flex-col justify-end">
        <div id="mobile-more-content" class="bg-gray-900 rounded-t-2xl border-t border-gray-700 p-6 transform translate-y-full transition-transform duration-300 ease-out pb-10">
            ...
            <div class="space-y-2 font-mono text-sm">
                <a href="report" class="flex items-center text-neon-green p-3 rounded bg-gray-800 transition-colors">
                    <svg class="w-5 h-5 mr-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg>
                    Annual Report
                </a>
                <a href="accounts" class="flex items-center text-gray-400 hover:text-white p-3 rounded hover:bg-gray-800 transition-colors">
