...
<body class="flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto relative pb-20 md:pb-0">
        <header class="h-16 bg-terminal-panel border-b border-gray-800 flex items-center justify-between px-4 md:px-6 shrink-0 sticky top-0 z-20">
            <div class="flex items-center space-x-4 md:space-x-6">
                ...
                <span class="bg-black border border-gray-700 text-electric-blue font-mono text-xs md:text-sm px-3 py-1 font-bold rounded">LEDGER: MANAGED FUNDS (PAMM) LOCKED</span>
            </div>
        ...
        <div class="p-4 md:p-6 flex-1 flex flex-col">
            <h1 class="text-lg md:text-xl font-bold font-mono text-gray-400 border-b border-gray-800 pb-2 mb-6 mt-2">DYNAMIC_PAMM_DISTRIBUTION_LEDGER (1-ON-1)</h1>
            
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-1">
                    <div class="bg-terminal-panel p-5 md:p-6 rounded border border-gray-800 shadow-lg">
                        <h2 class="text-electric-blue font-mono text-xs md:text-sm font-bold mb-6">[ WD CALCULATION EXECUTION ]</h2>
                ...
                <div class="lg:col-span-2">
                    <?php if($distribution_result): ?>
                        <div class="bg-terminal-panel p-4 md:p-6 rounded border border-electric-blue shadow-lg relative mt-6 lg:mt-0">
                            <div class="absolute top-0 right-0 bg-electric-blue text-black font-mono text-[10px] md:text-xs font-bold px-3 py-1 rounded-bl">CALCULATION: SECURED</div>
                            
                            <h2 class="text-gray-400 font-mono text-base md:text-lg font-bold mb-4 mt-4 md:mt-2 border-b border-gray-800 pb-2">1-ON-1 DISTRIBUTION LEDGER</h2>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                <div class="bg-gray-900 border border-gray-700 p-4 rounded">
                                    ...
                                    <div class="text-base md:text-lg font-bold font-mono text-white"><?= number_format($distribution_result['total_capital'], 2, '.', ',') ?> USC</div>
                                </div>
                                <div class="bg-gray-900 border border-gray-700 p-4 rounded">
                                    <div class="text-gray-500 font-mono text-xs mb-1">WD PROFIT TO SPLIT</div>
                                    <div class="text-base md:text-lg font-bold font-mono text-neon-green"><?= number_format($wd_amount, 2, '.', ',') ?> USC</div>
                                </div>
                            </div>
                            ...
                            <div class="bg-black rounded border border-gray-800 overflow-x-auto">
                                <table class="w-full text-left whitespace-nowrap md:whitespace-normal">
                                    <thead>
                                        <tr class="border-b border-gray-800 text-gray-500 font-mono text-[10px] md:text-xs">
                                            <th class="p-3 md:p-4 uppercase">Entity</th>
                                            <th class="p-3 md:p-4 text-right uppercase">Capital</th>
                                            <th class="p-3 md:p-4 text-right uppercase">Ratio</th>
                                            <th class="p-3 md:p-4 text-right text-white uppercase">Profit Share</th>
                                        </tr>
                                    </thead>
                                    <tbody class="font-mono text-xs md:text-sm">
                                        <tr class="border-b border-gray-800 transition-colors bg-gray-900">
                                            <td class="p-3 md:p-4 text-neon-green font-bold">TOMMY ALFARABI</td>
                                            <td class="p-3 md:p-4 text-right text-gray-400"><?= number_format($distribution_result['tommy_capital'], 2, '.', ',') ?></td>
                                            <td class="p-3 md:p-4 text-right text-neon-green font-bold"><?= $distribution_result['tommy_ratio'] ?>%</td>
                                            <td class="p-3 md:p-4 text-right text-neon-green font-bold text-base md:text-lg"><?= number_format($distribution_result['tommy_share'], 2, '.', ',') ?></td>
                                        </tr>
                                        
                                        <tr class="transition-colors hover:bg-gray-800">
                                            <td class="p-3 md:p-4 text-electric-blue font-bold"><?= htmlspecialchars($distribution_result['client_name']) ?></td>
                                            <td class="p-3 md:p-4 text-right text-gray-400"><?= number_format($distribution_result['client_capital'], 2, '.', ',') ?></td>
                                            <td class="p-3 md:p-4 text-right text-electric-blue font-bold"><?= $distribution_result['client_ratio'] ?>%</td>
                                            <td class="p-3 md:p-4 text-right text-electric-blue font-bold text-base md:text-lg"><?= number_format($distribution_result['client_share'], 2, '.', ',') ?></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                            ...
                            <?php if(!$distribution_result['has_client']): ?>
                                <div class="mt-4 text-warning-yellow text-[10px] md:text-xs font-mono text-center animate-pulse">
                    ...
                    <?php else: ?>
                        <div class="h-full min-h-[200px] mt-6 lg:mt-0 flex items-center justify-center border-2 border-dashed border-gray-800 rounded opacity-50">
                            <span class="text-gray-600 font-mono text-xs md:text-sm">AWAITING WD INPUT...</span>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <div class="md:hidden fixed bottom-0 w-full bg-black/80 backdrop-blur-lg border-t border-gray-800 z-50 flex justify-around items-center pt-2 pb-safe" style="padding-bottom: env(safe-area-inset-bottom, 12px);">
        ...
        <a href="distribution" class="flex flex-col items-center p-2 text-neon-green">
            <svg class="w-6 h-6 mb-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
